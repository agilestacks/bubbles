    ...
    stage('Build') {
      steps {
        container('toolbox') {
          dir('api') {
            script {
              final component = hub.explain(state: params.HUB_STATE).components['bubbles-ecr']
      ...
      steps {
        container('toolbox') {
          script {
            ...
            final name = 'bubbles'
            sh script: "kubectl -n ${namespace} --record 'deployment/${name}' set image 'bubbles=${image}'"
          }
        }
      }
    }
